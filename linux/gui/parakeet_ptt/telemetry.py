"""
Telemetry logger — appends one JSON line per event to telemetry.jsonl.
"""
...
def transcription_complete(
    inference_s: float, total_latency_s: float,
    word_count: int, char_count: int, corrections_applied: int,
    gpu_mem_used_mb: float, gpu_mem_reserved_mb: float, text: str,
):
    _emit(
